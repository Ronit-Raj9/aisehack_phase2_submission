"""
Baseline training from raw monthly ``.npy`` (no separate prepare_dataset stage).

Usage:
    python scripts/train.py --config configs/train.yaml
    python scripts/train.py --config configs/train.yaml --raw_path /path/to/raw
"""
...
if device.type == "cuda":
    try:
        ...
        raise RuntimeError(
            "CUDA runtime initialization failed. This usually means the current notebook "
            "kernel has a stale CUDA error state from a previous failed run. "
            "Restart the kernel and rerun from the first notebook cell. "
...
validate_every = int(getattr(cfg.training, "validate_every", 1))
if validate_every < 1:
    validate_every = 1
use_amp = bool(getattr(cfg.training, "use_amp", False)) and device.type == "cuda"
grad_clip_norm = float(getattr(cfg.training, "grad_clip_norm", 1.0))
scaler = torch.amp.GradScaler("cuda", enabled=use_amp)

# =========================================================
# Dataloader (raw windows + causal derived features)
# =========================================================
...
if device.type == "cuda" and mgpu_enabled and mgpu_backend == "data_parallel":
    ...
    if gpu_count >= mgpu_min_gpus:
        ...
        print(f"✅ DataParallel enabled on GPUs: {active_ids}")
        print("ℹ️ CuDNN benchmark disabled for multi-GPU stability")
    else:
        print(f"ℹ️ multi_gpu.enabled=True but only {gpu_count} GPU(s) available (< min_gpus={mgpu_min_gpus}); running single-GPU.")
...
# =========================================================
# Checkpoint paths — best.pt + last.pt convention
# =========================================================
...
save_every_checkpoint = bool(getattr(cfg.training, "save_every_checkpoint", False))
resume_enabled   = getattr(cfg.training, "resume_from_best", True)
...
log_competition_metrics = getattr(cfg.training, "log_competition_metrics", True)
...
checkpoint_on_offline_score = getattr(cfg.training, "checkpoint_on_offline_score", False)
...
def alpha_for_epoch(epoch: int) -> float:
    if epoch >= episode_finetune_start_epoch:
        return episode_alpha_finetune
    if epoch >= episode_main_start_epoch:
        return episode_alpha_main
    return episode_alpha_warmup
...
def sync_live_artifacts(epoch: int, force: bool = False) -> None:
    ...
    try:
        ...
        if copied:
            print(f" 🔁 live sync → {live_sync_dir}", end="", flush=True)
    except Exception as exc:
        print(f" ⚠️ live sync failed: {exc}", end="", flush=True)

# =========================================================
# Training (same loop math as baseline)
# =========================================================
...
if save_every_checkpoint:
    print("Periodic epoch checkpoints: enabled")
...
for ep in tqdm(range(start_epoch, epochs), desc="Epoch", initial=start_epoch, total=epochs):
    ...
    for batch in train_loader:
        ...
        amp_ctx = torch.amp.autocast(device_type="cuda", enabled=use_amp) if device.type == "cuda" else contextlib.nullcontext()
        with amp_ctx:
            if model_name in ("hybrid", "ensemble", "hybrid_ensemble"):
                out, aux = model(x, season_idx=season_idx, return_aux=True)
                hist_cpm = x[..., 0]
                ep_mask = cmetrics.episode_proxy_mask(
                    hist_cpm, y, sigma=episode_sigma, floor=episode_floor
                )
    ...
    # ── Validate ─────────────────────────────────────────────
    do_validate = ((ep - start_epoch) % validate_every == 0) or (ep == epochs - 1)
    model.eval()
    test_l2 = float("nan")
    comp_sums = {
        k: 0.0
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                if log_competition_metrics:
                    ...
                    bundle = cmetrics.competition_metric_bundle(
                        out, y, ep_mask, w1=metric_w1, w2=metric_w2, w3=metric_w3
                    )
                    bs = x.size(0)
                    comp_batches += bs
                    for k in comp_sums:
    ...
    log_row = {
        ...
        "duration": dur,
        "train_l2": train_l2,
        "val_l2": test_l2 if do_validate else None,
    }
    ...
    if do_validate and log_competition_metrics and comp_batches > 0:
        ...
        print(f"Ep {ep:3d} | {dur:.0f}s | train={train_l2:.5f} | val={test_l2:.5f}", end="", flush=True)
    else:
        print(f"Ep {ep:3d} | {dur:.0f}s | train={train_l2:.5f} | val=skipped (every {validate_every})", end="", flush=True)

    # ── WandB (optional, only if active) ─────────────────────
    try:
        import wandb
        if wandb.run is not None:
            wb = {"epoch": ep, "train_l2": train_l2, "duration_s": dur, "lr": optimizer.param_groups[0]["lr"]}
            if do_validate:
                wb["val_l2"] = test_l2
            if do_validate and log_competition_metrics and comp_batches > 0:
    ...
    # ── Save best (val L2 and/or offline surrogate) ──────────
    improved_l2 = False
    improved_surrogate = False
    if do_validate:
    ...
    if checkpoint_on_offline_score:
        save_best = do_validate and improved_surrogate
    else:
        save_best = do_validate and improved_l2
    ...
    # ── Save last (always, overwrite previous) ───────────────
    save_checkpoint(last_ckpt_path, ep, test_l2)
    sync_live_artifacts(ep)

    # ── Periodic checkpoint & log dump ───────────────────────
    if (ep + 1) % checkpoint_every == 0:
        if save_every_checkpoint:
            epoch_ckpt_path = os.path.join(ckpt_dir, f"epoch_{ep + 1:03d}.pt")
            save_checkpoint(epoch_ckpt_path, ep, test_l2)
            if live_sync_enabled:
                try:
                    os.makedirs(live_sync_dir, exist_ok=True)
                    shutil.copy2(epoch_ckpt_path, os.path.join(live_sync_dir, os.path.basename(epoch_ckpt_path)))
                except Exception as exc:
                    print(f" ⚠️ epoch checkpoint live copy failed: {exc}", end="", flush=True)
            print(f" 💾 {os.path.basename(epoch_ckpt_path)}", end="", flush=True)

        with open(log_save, "w") as f:
            json.dump(log, f)
        sync_live_artifacts(ep)
        print(f" 📝 log saved", end="", flush=True)
